...
import type { AgentLogEvent, Level } from '../models/event.js'
...
export class McpNotifier {
  ...
  constructor(options: { server: Server; enabled: boolean; minLevel: Level }) {
    const { server, enabled, minLevel } = options
    this.server = server
  ...
  shouldNotify(event: AgentLogEvent): boolean {
    return this.enabled && (event.phase === 'DECISION' || levelRank(event.level) >= levelRank(this.minLevel))
  }
